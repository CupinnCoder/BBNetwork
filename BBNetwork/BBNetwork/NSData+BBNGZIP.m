...
- (NSData *)bbn_gzippedDataWithCompressionLevel:(float)level
{
    if (self.length == 0 || [self bbn_isGzippedData])
    {
        return self;
    }
...
- (NSData *)bbn_gzippedData
{
    return [self bbn_gzippedDataWithCompressionLevel:-1.0f];
}

- (NSData *)bbn_gunzippedData
{
    if (self.length == 0 || ![self bbn_isGzippedData])
    {
        return self;
